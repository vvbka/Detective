...
module.exports = function (room) {
    ...
    */
    ...
    if($global.master.Guess.room.first().prob === 1){cardset.push($global.master.Guess.room.first().itm)}
    if($global.master.Guess.person.first().prob === 1){cardset.push($global.master.Guess.person.first().itm)}
    if($global.master.Guess.weapon.first().prob === 1){cardset.push($global.master.Guess.weapon.first().itm)}
    ...
    if(ask.person === undefined || ask.room === undefined || ask.weapon === undefined){
        ...
        //AAAND we're done (if we have all the elements of a question)
        this.done(null, ask);
    }
